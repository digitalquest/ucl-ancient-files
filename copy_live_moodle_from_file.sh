...
function usage() {
	...
	Usage:
	  ...
	  copy_live_moodle_from_file.sh [--skip-logs] <dev database name> <backup filename>

	Loads a copy of the UCL Moodle live database into a local dev database,
        as specified in first command line argument. Uses local tar.gz backup file
        specified in <backup filename> argument, filters it for dev/UAT environment and
        loads it into the specified local database. May need the database user to be
        updated in the .sh file, look for the dev_db_user variable.
...
backup_file_provided=1
# get that backup filename
([[ -n $1 ]] && backup_filename=$1) || {
    echo No local backup filename provided; will get the file from the backup server.
    echo
    backup_file_provided=0
}
...
if [[ $backup_file_provided ==1 ]]; then {
    if [[ ${include_logs} == 1 ]]; then {
        gzip -d -c /data/mysql/backup/${backup_filename} | \
